...
test("Faktoren: genau 14 Module, IDs eindeutig und vollständig", () => {
  ...
  for (const id of FACTOR_IDS) {
    assert.equal(
      FACTORS[id].id,
      id,
      `Faktor ${id} muss seine eigene ID tragen`,
    );
...
test("liquidity: 1 Mio. Quote-Volumen ⇒ log-normiert 0.2 (Golden)", () => {
  const v = FACTORS.liquidity.compute(
    input({ instrument: instrument({ volume24h: 1_000_000 }) }),
  );
...
test("liquidity: ohne Registry-Volumen greift das Quote-Volumen der letzten Kerze", () => {
  ...
  const v = FACTORS.liquidity.compute(
    input({ instrument: instrument({ volume24h: null }), candles }),
  );
  assert.equal(v.raw, 1_000_000);
...
test("liquidity: kein Volumen ⇒ unavailable mit Neutralwert 0", () => {
  const v = FACTORS.liquidity.compute(
    input({ instrument: instrument({ volume24h: null }) }),
  );
...
test("spread: 10 bp ⇒ 0.8163265306 (Golden, invers linear)", () => {
  const v = FACTORS.spread.compute(
    input({ instrument: instrument({ spread: 0.001 }) }),
  );
...
test("spread: Grenzen — bestSpread ⇒ 1, worstSpread ⇒ 0, unbekannt ⇒ unavailable", () => {
  assert.equal(
    FACTORS.spread.compute(
      input({ instrument: instrument({ spread: 0.0001 }) }),
    ).normalized,
    1,
  );
  assert.equal(
    FACTORS.spread.compute(input({ instrument: instrument({ spread: 0.005 }) }))
      .normalized,
    0,
  );
  const unknown = FACTORS.spread.compute(
    input({ instrument: instrument({ spread: null }) }),
  );
...
test("atr: zu wenig Kerzen ⇒ unavailable", () => {
  const v = FACTORS.atr.compute(
    input({ candles: candlesFromCloses([100, 101, 102]) }),
  );
  assert.equal(v.available, false);
  assert.equal(v.normalized, 0);
});
...
test("volatility: alternierende Kurse 100/110 ⇒ σ 0.09531018, annualisiert 1.8208984284 (Golden)", () => {
  const closes = Array.from({ length: 11 }, (_, i) =>
    i % 2 === 0 ? 100 : 110,
  );
  const v = FACTORS.volatility.compute(
    input({ candles: candlesFromCloses(closes) }),
  );
...
test("volatility: konstante Kurse ⇒ σ = 0 ⇒ normalisiert 0 (unterhalb floor)", () => {
  const v = FACTORS.volatility.compute(
    input({ candles: candlesFromCloses(new Array(40).fill(100)) }),
  );
...
test("volatility: leere Serie und Einzelwert ⇒ unavailable", () => {
  assert.equal(
    FACTORS.volatility.compute(input({ candles: [] })).available,
    false,
  );
  assert.equal(
    FACTORS.volatility.compute(input({ candles: candlesFromCloses([100]) }))
      .available,
    false,
  );
});
...
test("momentum: 0,1 % Tageswachstum über 61 Kerzen ⇒ 0.037961702 (Golden)", () => {
  const v = FACTORS.momentum.compute(
    input({ candles: candlesFromCloses(growthSeries(100, 1.001, 61)) }),
  );
...
test("momentum: Abwärtsbewegung zählt im Default-Modus absolut, directional dagegen 0", () => {
  ...
  const directionalConfig = resolveScannerConfig({
    factors: { momentum: { mode: "directional" } },
  });
  const directional = FACTORS.momentum.compute(
    input({ candles, config: directionalConfig }),
  );
...
test("momentum: zu kurze Serie ⇒ unavailable", () => {
  assert.equal(
    FACTORS.momentum.compute(input({ candles: candlesFromCloses([100, 101]) }))
      .available,
    false,
  );
});
...
test("trend: konstante Kurse ⇒ raw 0, keine Struktur ⇒ 0 (Golden)", () => {
  const v = FACTORS.trend.compute(
    input({ candles: candlesFromCloses(new Array(60).fill(100)) }),
  );
  assert.equal(v.raw, 0);
  assert.equal(v.normalized, 0);
...
test("trend: sauberer Aufwärtstrend ⇒ aligned und > 0.5", () => {
  const v = FACTORS.trend.compute(
    input({ candles: candlesFromCloses(growthSeries(100, 1.01, 80)) }),
  );
...
test("volumeRatio: 5×200 gegen 20er-Schnitt 125 ⇒ 1.6 / 0.7333333333 (Golden)", () => {
  ...
  const v = FACTORS.volumeRatio.compute(
    input({
      candles: candlesFromCloses(new Array(20).fill(100), { volume: volumes }),
    }),
  );
...
test("volumeRatio: Nullvolumen ⇒ unavailable, negatives Volumen ⇒ unavailable", () => {
  const zero = FACTORS.volumeRatio.compute(
    input({
      candles: candlesFromCloses(new Array(20).fill(100), { volume: 0 }),
    }),
  );
  ...
  assert.equal(
    FACTORS.volumeRatio.compute(input({ candles: negative })).available,
    false,
  );
});
...
test("rsi: reiner Aufwärtslauf ⇒ 100 ⇒ Überhitzungsfilter 0 (Golden)", () => {
  const v = FACTORS.rsi.compute(
    input({ candles: candlesFromCloses(growthSeries(100, 1.01, 30)) }),
  );
...
test("rsi: konstante Kurse ⇒ 50 ⇒ 1 (voll im neutralen Band)", () => {
  const v = FACTORS.rsi.compute(
    input({ candles: candlesFromCloses(new Array(30).fill(100)) }),
  );
...
test("rsi: Wilder-Referenz — ±1 im Wechsel, letzter Schritt aufwärts ⇒ 50.664722", () => {
  ...
  for (let i = 1; i <= 20; i++)
    mirrored.push(mirrored[i - 1] + (i % 2 === 0 ? -1 : 1));
...
test("drawdown: 120 → 60 ⇒ 50 % ⇒ normalisiert 0 (Golden)", () => {
  const v = FACTORS.drawdown.compute(
    input({ candles: candlesFromCloses([100, 120, 60, 80]) }),
  );
...
test("drawdown: monoton steigende Kurse ⇒ 0 ⇒ normalisiert 1", () => {
  const v = FACTORS.drawdown.compute(
    input({ candles: candlesFromCloses([100, 110, 120, 130]) }),
  );
  assert.equal(v.raw, 0);
  assert.equal(v.normalized, 1);
});
...
test("drawdown: Einzelwert ⇒ unavailable", () => {
  assert.equal(
    FACTORS.drawdown.compute(input({ candles: candlesFromCloses([100]) }))
      .available,
    false,
  );
});
...
test("correlation: identische Serien ⇒ r = 1 ⇒ Diversifikationsnutzen 0 (Golden)", () => {
  ...
  const v = FACTORS.correlation.compute(
    input({ candles, benchmarkCandles: candles }),
  );
...
test("correlation: gegenläufige Serien ⇒ r = −1 ⇒ Nutzen 0 (Betrag zählt)", () => {
  ...
  const v = FACTORS.correlation.compute(
    input({
      ...
      benchmarkCandles: candlesFromCloses(inverse),
    }),
  );
  assert.equal(v.raw, -1);
  assert.equal(v.normalized, 0);
...
test("correlation: ohne Benchmark ⇒ unavailable mit Neutralwert 0.5", () => {
  const v = FACTORS.correlation.compute(
    input({ candles: candlesFromCloses(growthSeries(100, 1.01, 40)) }),
  );
  assert.equal(v.available, false);
...
test("news: veraltete Registry-Daten erhöhen das Risiko deterministisch", () => {
  const stale = instrument({
    lastSeen: new Date(AS_OF_MS - 10 * 86_400_000).toISOString(),
  });
  const v = FACTORS.news.compute(
    input({
      instrument: stale,
...
test("funding: Perpetual ohne Rate ⇒ unavailable mit 0.5", () => {
  const v = FACTORS.funding.compute(
    input({ instrument: instrument({ marketType: "perpetual" }) }),
  );
  assert.equal(v.available, false);
  assert.equal(v.normalized, 0.5);
});
...
test("executionCost: 2×4 bp Taker + 2 bp Spread ⇒ 10 bp ⇒ 0.8888888889 (Golden)", () => {
  const v = FACTORS.executionCost.compute(
    input({ instrument: instrument({ takerFee: 0.0004, spread: 0.0002 }) }),
  );
  assert.equal(v.raw, 0.001);
...
test("executionCost: feeMode maker/blend rechnen mit den Registry-Gebühren", () => {
  ...
  const maker = resolveScannerConfig({
    factors: { execution: { feeMode: "maker" } },
  });
  const blend = resolveScannerConfig({
    factors: { execution: { feeMode: "blend" } },
  });
  assert.equal(
    FACTORS.executionCost.compute(input({ instrument: inst, config: maker }))
      .raw,
    0.0002,
  );
  assert.equal(
    FACTORS.executionCost.compute(input({ instrument: inst, config: blend }))
      .raw,
    0.0006,
  );
});
...
test("Edge Cases: leere Serie liefert für jeden Faktor einen endlichen Wert in [0,1]", () => {
  ...
  for (const id of FACTOR_IDS) {
    const v = values[id];
    assert.ok(
      Number.isFinite(v.normalized),
      `${id}: normalized muss endlich sein`,
    );
    assert.ok(
      v.normalized >= 0 && v.normalized <= 1,
      `${id}: normalized außerhalb [0,1]`,
    );
    assert.ok(
      v.raw === null || Number.isFinite(v.raw),
      `${id}: raw muss null oder endlich sein`,
    );
...
test("Edge Cases: NaN-Preise werfen nie und führen nie zu NaN-Ausgaben", () => {
  ...
  for (const id of FACTOR_IDS) {
    assert.ok(
      Number.isFinite(values[id].normalized),
      `${id}: NaN durchgereicht`,
    );
  }
});
...
test("Determinismus: zwei Berechnungen derselben Eingabe sind identisch", () => {
  const candles = candlesFromCloses(growthSeries(100, 1.003, 90), {
    wickPct: 0.01,
  });
...
// ── Annualisierung aus dem echten Kerzenintervall (v1.37.0) ─────────────────

test("median: ungerade/gerade Laenge, leer, Reihenfolge-unabhaengig", () => {
  ...
  assert.equal(
    median([1, 1, 1, 100]),
    1,
    "Ausreisser verschieben den Median nicht",
  );
});

test("inferPeriodsPerYear: leitet Tages-/Stundenkerzen korrekt ab und faellt robust zurueck", () => {
  const day = 86_400_000;
  const hour = 3_600_000;
  const dayStamps = Array.from({ length: 31 }, (_, i) => i * day);
  const hourStamps = Array.from({ length: 31 }, (_, i) => i * hour);
  assert.equal(Math.round(inferPeriodsPerYear(dayStamps, 8760)), 365);
  assert.equal(Math.round(inferPeriodsPerYear(hourStamps, 365)), 8760);
  // Luecken (Wochenenden/Ausfaelle) verzerren den Median nicht.
  const gappy = [0, hour, 2 * hour, 26 * hour, 27 * hour, 28 * hour];
  assert.equal(Math.round(inferPeriodsPerYear(gappy, 365)), 8760);
  // Zu wenige/ungueltige Abstaende => Fallback.
  assert.equal(inferPeriodsPerYear([0], 365), 365);
  assert.equal(
    inferPeriodsPerYear([0, 5_000], 365),
    365,
    "Abstand unter 1 Minute => Fallback",
  );
  assert.equal(
    inferPeriodsPerYear([0, 31 * day], 365),
    365,
    "Abstand ueber 1 Woche => Fallback",
  );
});

test("volatility: 1h-Kerzen werden mit sqrt(8760) annualisiert (nicht mehr mit sqrt(365))", () => {
  // Realistische 1h-Range von +/-0,5 %: sigma_h ~ ln(1,005) = 0,00499.
  // Korrekt annualisiert ~ 0,47 (Idealband 0,2...0,8); mit der alten
  // 365-Skalierung waeren es ~0,095 geworden — unterhalb des Idealkorridors
  // und damit ein nahezu wirkungsloser Scoreblock (Hauptbefund des Reviews).
  const closes = Array.from({ length: 31 }, (_, i) =>
    i % 2 === 0 ? 100 : 100.5,
  );
  const candles = candlesFromCloses(closes, { stepMs: 3_600_000 });
  const v = FACTORS.volatility.compute(input({ candles }));
  assert.equal(v.detail.periodsPerYear, 8760);
  assert.equal(v.detail.inferred, true);
  assert.ok(
    (v.raw as number) > 0.4 && (v.raw as number) < 0.6,
    `annualisiert ${v.raw} sollte ~0,47 sein`,
  );
  assert.equal(v.normalized, 1, "korrekt skalierte Reihe liegt im Idealband");

  // Alte Fehlskalierung zum Vergleich: dieselben Stundenkerzen mit 365
  // gedanklich skaliert landen unter idealLow (Scoreflanke, nicht Ideal).
  const legacy = FACTORS.volatility.compute(
    input({
      candles,
      config: resolveScannerConfig({
        factors: {
          volatility: { inferPeriodsPerYear: false, periodsPerYear: 365 },
        },
      }),
    }),
  );
  assert.equal(legacy.detail.periodsPerYear, 365);
  assert.ok(
    (legacy.normalized as number) < 1,
    "falsch skalierte Reihe erreicht den Idealwert nicht",
  );
});

test("volatility: inferPeriodsPerYear=false nutzt den Config-Fallback (Default jetzt 1h = 8760)", () => {
  const cfg = resolveScannerConfig({
    factors: { volatility: { inferPeriodsPerYear: false } },
  });
  const closes = Array.from({ length: 31 }, (_, i) =>
    i % 2 === 0 ? 100 : 110,
  );
  const candles = candlesFromCloses(closes, { stepMs: 86_400_000 });
  const v = FACTORS.volatility.compute(input({ candles, config: cfg }));
  assert.equal(v.detail.inferred, false);
  assert.equal(
    v.detail.periodsPerYear,
    8760,
    "expliziter Opt-out nutzt den Config-Wert, jetzt Default 8760",
  );
});
