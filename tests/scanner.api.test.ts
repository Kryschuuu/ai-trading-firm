...
import {
  GET as dailyGET,
  parseDailyQuery,
} from "../src/app/api/universe/daily/route";
import {
  GET as weeklyGET,
  parseWeeklyQuery,
} from "../src/app/api/universe/weekly/route";
import {
  GET as scoreGET,
  parseInstrumentId,
} from "../src/app/api/universe/score/[instrumentId]/route";
import {
  ScannerService,
...
test("GET /api/universe/daily liefert Trichter-Ebenen inklusive Breakdown", async () => {
  ...
  // v1.37.0: die Readiness ist Teil des Payloads (UI trennt „keine Chance“
  // von „keine Daten“). Alle fünf Service-Instrumente erhalten hier Kerzen.
  assert.equal(
    (body as { readiness?: { status: string } }).readiness?.status,
    "READY",
  );
...
test("GET /api/universe/daily: Ebenen deep/interesting/eligible sind wählbar", async () => {
  for (const level of ["deep", "interesting", "eligible"]) {
    const res = await dailyGET(
      new Request(`${BASE}/api/universe/daily?level=${level}`),
    );
    assert.equal(res.status, 200);
    const body = (await res.json()) as {
      level: string;
      items: { breakdown?: unknown }[];
    };
    assert.equal(body.level, level);
    if (level !== "deep") {
      assert.equal(
        body.items[0]?.breakdown,
        undefined,
        `${level} darf keinen Breakdown liefern`,
      );
    }
  }
});
...
test("GET /api/universe/daily: Pagination und harte Größenlimits", async () => {
  const page1 = await dailyGET(
    new Request(`${BASE}/api/universe/daily?pageSize=1&page=1`),
  );
  ...
  const page2 = await dailyGET(
    new Request(`${BASE}/api/universe/daily?pageSize=1&page=2`),
  );
  const body2 = (await page2.json()) as { items: { rank: number }[] };
  assert.equal(body2.items[0].rank, 2);

  const tooLarge = await dailyGET(
    new Request(`${BASE}/api/universe/daily?pageSize=5000`),
  );
  assert.equal(tooLarge.status, 400);
  const err = (await tooLarge.json()) as {
    ok: boolean;
    error: string;
    message: string;
  };
...
test("GET /api/universe/daily: unsinnige Parameter ⇒ 400 statt 500", async () => {
  for (const query of [
    ...
    "pageSize=0",
    "pageSize=-3",
    "breakdown=vielleicht",
  ]) {
    const res = await dailyGET(
      new Request(`${BASE}/api/universe/daily?${query}`),
    );
...
test("parseDailyQuery: Defaults und Breakdown-Regeln", () => {
  ...
  assert.equal(
    parseDailyQuery(new URL(`${BASE}/x?level=eligible`)).breakdown,
    false,
  );
  assert.equal(
    parseDailyQuery(new URL(`${BASE}/x?breakdown=false`)).breakdown,
    false,
  );
  assert.equal(
    parseDailyQuery(new URL(`${BASE}/x?level=interesting&breakdown=true`))
      .breakdown,
    false,
  );
});
...
test("GET /api/universe/weekly liefert Klassifikation und Zusammenfassung", async () => {
  ...
  for (const item of body.items) {
    ...
    assert.ok(
      ["CORE", "ROTATION", "DISCOVERY", "EXCLUDED"].includes(item.class),
    );
  }
  assert.equal(
...
test("GET /api/universe/weekly: Klassenfilter und Validierung", async () => {
  const res = await weeklyGET(
    new Request(`${BASE}/api/universe/weekly?class=EXCLUDED`),
  );
  const body = (await res.json()) as { items: { class: string }[] };
  assert.ok(body.items.length > 0);
  assert.ok(body.items.every((i) => i.class === "EXCLUDED"));

  const multi = await weeklyGET(
    new Request(`${BASE}/api/universe/weekly?class=EXCLUDED,DISCOVERY`),
  );
  assert.equal(multi.status, 200);

  for (const query of ["class=VIP", "class=", "page=0", "pageSize=999"]) {
    const bad = await weeklyGET(
      new Request(`${BASE}/api/universe/weekly?${query}`),
    );
    assert.equal(bad.status, 400, `erwartete 400 für ?${query}`);
  }
  assert.deepEqual(parseWeeklyQuery(new URL(`${BASE}/x`)), {
    classes: null,
    page: 1,
    pageSize: 50,
  });
...
test("GET /api/universe/score/{id} liefert vollständigen Breakdown", async () => {
  const res = await scoreGET(
    new Request(`${BASE}/api/universe/score/BINANCE:BTCUSDT`),
    {
  ...
  const body = (await res.json()) as {
    ...
    score: {
      ...
      score: number;
      regime: string;
      breakdown: { component: string; contribution: number }[];
...
test("GET /api/universe/score/{id}: abgelehntes Instrument zeigt die Filterregel", async () => {
  const res = await scoreGET(
    new Request(`${BASE}/api/universe/score/BINANCE:JUNKUSDT`),
    {
...
test("GET /api/universe/score/{id}: unbekannt ⇒ 404, kaputte ID ⇒ 400", async () => {
  const notFound = await scoreGET(
    new Request(`${BASE}/api/universe/score/BINANCE:NOSUCH`),
    {
  ...
  assert.equal(
    ((await notFound.json()) as { error: string }).error,
    "NOT_FOUND",
  );
...
test("Service: refresh() rechnet neu, Weekly folgt dem aktuellen Scan", () => {
  ...
  assert.equal(
    svc.getScan(),
    first,
    "zweiter Zugriff nutzt das gecachte Ergebnis",
  );
  ...
  assert.equal(
    svc.scoreFor("BINANCE:BTCUSDT")?.instrumentId,
    "BINANCE:BTCUSDT",
  );
...
test("Service: eine engere Konfiguration verkleinert die Ebenen sichtbar", async () => {
  setScannerServiceForTests(
    service(
      resolveScannerConfig({ funnel: { dailyMax: 1, deepMax: 1, deepMin: 1 } }),
    ),
  );
  const res = await dailyGET(new Request(`${BASE}/api/universe/daily`));
  const body = (await res.json()) as {
    funnel: { daily: number; deep: number };
    total: number;
  };
