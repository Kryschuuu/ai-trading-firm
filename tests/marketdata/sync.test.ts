...
test("sync upserts discovered instruments into registry", async () => {
  const { adapter } = mockMarketDataAdapter({
    instruments: [instrumentOf("BTCUSDT"), instrumentOf("ETHUSDT")],
  });
  ...
  assert.ok(
    registry.size > 0,
    "Registry muss nach dem Sync Instrumente enthalten",
  );
...
test("market sync enriches 24h volume", async () => {
  ...
  assert.ok(btc);
  assert.ok(
    btc!.volume24h !== null && btc!.volume24h > 0,
    `volume24h > 0 erwartet, war ${btc!.volume24h}`,
  );
  assert.equal(
    btc!.volume24h,
    42_500_000,
  ...
  assert.equal(
    ...
    "lastSeen wird beim Sync gestempelt",
  );
});
...
test("market sync calculates spread from best bid/ask", async () => {
  ...
  assert.ok(
    Math.abs(btc!.spread! - 0.00019998) < 1e-8,
  ...
  assert.equal(
    emptyResult.spreadsUnknown,
    1,
    "ein Instrument mit unbekanntem Spread",
  );
  assert.equal(
    emptyResult.degraded,
    false,
    "bekannter Spread-missing ist Data-Quality, kein Fehler",
  );
});
...
test("sync persists candles per timeframe", async () => {
  ...
  assert.ok(
    ...
    "Bars müssen ts-aufsteigend sein",
  );
});
...
test("sync uses one bulk ticker call, not one per instrument", async () => {
  ...
  assert.equal(
    calls.tickerBatches.length,
    1,
    "genau EIN Bulk-Ticker-Call für 6 Instrumente",
  );
  assert.equal(
    calls.orderBook.length,
    6,
    "N × depth (Spread geht nur aus dem Orderbook)",
  );
  assert.deepEqual(
    calls.ticker,
    [],
    "kein per-Symbol-getTicker bei vollständigem Bulk",
  );
  assert.equal(
    calls.candles.length,
    6 * SYNC_TIMEFRAMES.length,
    `N × M × kline (M = ${SYNC_TIMEFRAMES.length} Default-Timeframe(s), SYNC_TIMEFRAMES)`,
  );
...
test("adapter failure on single symbol does not abort sync (continueOnError)", async () => {
  ...
  assert.equal(
    result.degraded,
    true,
    "Lauf ist degradiert, nicht fehlgeschlagen",
  );
  assert.equal(
    result.failures.length,
    1,
    `genau 1 isolierter Fehler, war ${result.failures.length}`,
  );
  ...
  assert.ok(
    calls.candles.length >= 5 * SYNC_TIMEFRAMES.length,
    "ein kline-Call je Instrument und Default-Timeframe",
  );
...
test("continueOnError=false bricht mit SyncPartialFailureError ab", async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) => {
      ...
      assert.ok(
        error.failureCount >= SYNC_TIMEFRAMES.length,
        `mind. ${SYNC_TIMEFRAMES.length} Timeframe-Fehler(n) (Default SYNC_TIMEFRAMES), war ${error.failureCount}`,
      );
      assert.ok(
        error.failures.length >= 1 && error.failures.length <= 10,
        "Fehlerliste ist gekürzt, aber nicht leer",
      );
      return true;
    },
  );
...
test("maxInstruments cap is deterministic and respected", async () => {
  const instruments = [
    ...
    "DDDUSDT",
    "EEEUSDT",
  ].map((symbol) => instrumentOf(symbol));
  ...
  const { adapter } = mockMarketDataAdapter({
    instruments,
    quoteVolOf: (symbol) => volume[symbol] ?? 0,
  });
  ...
  assert.deepEqual(
    ids,
    ["BITUNIX:BBBUSDT", "BITUNIX:DDDUSDT"],
    "die zwei liquidesten Instrumente bleiben",
  );
...
test("ohne Ticker wird alphabetisch nach Symbol gekappt", () => {
  const instruments = ["ZZZUSDT", "MMMUSDT", "AAAUSDT", "BBBUSDT"].map(
    (symbol) => instrumentOf(symbol),
  );
...
test("candleLimit below requiredWarmupCandles is rejected with actionable error", () => {
  ...
  assert.throws(
    () =>
      new MarketDataSyncService(registry, history, new Map(), {
        candleLimit: 5,
      }),
    InsufficientCandleLimitError,
  );
...
test("sync is idempotent: second run does not duplicate bars", async () => {
  ...
  assert.equal(
    barsAfter,
    barsBefore,
    `Bars dürfen nicht akkumulieren (${barsBefore} → ${barsAfter})`,
  );
  assert.equal(
    second.candlesByTimeframe["1h"]?.bars,
    0,
...
test("syncAll() läuft über alle registrierten Venues, sequentiell und sortiert", async () => {
  ...
  const alpha = mockMarketDataAdapter({
    instruments: [instrumentOf("BTCUSDT", "ALPHA")],
    venue: "ALPHA",
  });
  const beta = mockMarketDataAdapter({
    instruments: [instrumentOf("ETHUSDT", "BETA")],
    venue: "BETA",
  });
  ...
  assert.deepEqual(
    ...
    ["ALPHA", "BETA"],
    "sortierte Venue-Reihenfolge (deterministisch)",
  );
  assert.equal(
    results.reduce((sum, r) => sum + r.synced, 0),
    2,
  );
  assert.deepEqual(
    registry
...
test("SyncResult ist vollständig JSON-serialisierbar (roundtrip-stabil)", async () => {
  ...
  for (const key of ["venue", "startedAt", "finishedAt"])
    assert.equal(typeof record[key], "string", `${key} ist ein String`);
  for (const key of [
    "discovered",
    "synced",
    "skipped",
  ...
  assert.equal(
    result.degraded,
    false,
    "Kappung ist kein Fehler — nur failures degradieren",
  );
...
test("Logformat: discovery/tickers/orderbooks/candles-Zeilen mit Zählern", async () => {
  ...
  assert.ok(
    lines.includes("[market-sync] BITUNIX discovery: 4 instruments"),
    lines.join("\n"),
  );
  ...
  assert.equal(
    lines.some((l) => l.includes("SYM000USDT")),
    false,
...
test("symbolAllowlist begrenzt die Synchronisation auf die genannten Symbole", async () => {
  ...
  assert.equal(
    calls.orderBook.length,
    2,
    "Depth nur für die Allowlist-Symbole",
  );
});
...
test("ungültige Discovery-Symbole werden abgelehnt, bevor sie in eine URL gelangen", async () => {
  ...
  const { adapter } = mockMarketDataAdapter({
    instruments: [hostile, instrumentOf("ETHUSDT")],
  });
  const { service, registry } = syncHarness(adapter);
  ...
  assert.equal(
    result.discovered,
    2,
    "discovered zählt die Venue-Zeilen, nicht die brauchbaren",
  );
  assert.equal(result.synced, 1);
  assert.equal(
    result.skipped,
    1,
    "die abgelehnte Zeile ist als skipped sichtbar",
  );
  assert.ok(
    result.failures.some(
      (f) => f.stage === "discovery" && f.reason === "INVALID_SYMBOL",
    ),
...
test("Timeframe-Validierung: gemischte Reihen werden vor dem ersten Request abgelehnt", () => {
  ...
  assert.throws(
    () => resolveSyncOptions({ timeframes: [] }, 61),
    /darf nicht leer sein/,
  );
  assert.throws(
    () => resolveSyncOptions({ timeframes: ["5m", "5m"] }, 61),
    /doppelt/,
  );
  assert.throws(
    () => resolveSyncOptions({ maxInstruments: 0 }, 61),
    /maxInstruments muss eine positive Ganzzahl/,
  );
  assert.throws(
    () => resolveSyncOptions({ symbolAllowlist: ["BTC USDT?"] }, 61),
    /Symbol-Allowlist/,
  );
});
...
test("HistoricalStore-Schlüssel == Registry-Schlüssel (der Kern des Fehlers)", async () => {
  ...
  const { adapter } = mockMarketDataAdapter({
    instruments: [{ ...instrumentOf("BTCUSDT"), id: "bitunix:btcusdt" }],
  });
  const { service, registry, history } = syncHarness(adapter);
  ...
  assert.equal(
    stored[0].id,
    "BITUNIX:BTCUSDT",
    "Venue/Symbol werden kanonisch gehoben",
  );
  assert.equal(
