...
function ticker(
  symbol: string,
  quoteVol: number | null = 1_000_000,
  baseVol: number | null = null,
): MarketTicker {
  return {
    symbol,
    price: 100,
...
function mockAdapter(opts: {
  ...
  calls: { tickers: number; depth: string[]; ticker: string[] };
} {
  ...
  const adapter: MarketDataAdapter = {
    ...
    async getOrderBook(symbol) {
      ...
      if (opts.failDepthFor?.has(symbol))
        throw new Error(`depth failed for ${symbol}`);
...
test("market sync calculates spread from best bid/ask", () => {
  ...
  assert.ok(
    Math.abs(spread - 0.00019998) < 1e-8,
    `erwartet ≈0.00019998, war ${spread}`,
  );
});
...
test("missing ticker entry yields null volume, not an exception", async () => {
  ...
  const { volumeBySymbol, report } = await enrichWithTickers(
    instruments,
    adapter,
  );
  ...
  assert.deepEqual(
    calls.ticker,
    ["ETHUSDT"],
    "Lücken-Fallback per Einzel-Ticker",
  );
  assert.equal(
    volumeBySymbol.get("ETHUSDT"),
    1_000_000,
    "Fallback schließt die Bulk-Lücke",
  );
  ...
  assert.ok(
    second.report.missing.includes("BITUNIX:ETHUSDT") ||
      second.report.missing.includes("ETHUSDT"),
  );
  assert.ok(
...
test("depth failure for one symbol does not abort enrichment", async () => {
  ...
  const { spreadBySymbol, report } = await enrichWithOrderBooks(
    instruments,
    adapter,
    {
      depthLimit: 5,
...
test("crossed book (ask < bid) yields spread=null and a warning", async () => {
  ...
  assert.ok(
    warnings[0].toLowerCase().includes("crossed") ||
      warnings[0].includes("BTCUSDT"),
  );
});
...
test("implausible spread > 50% is rejected as null", async () => {
  ...
  assert.equal(
    spreadBySymbol.get("BTCUSDT"),
    null,
    "implausible spread must be null",
  );
...
test("tickers are fetched exactly once regardless of instrument count", async () => {
  ...
  assert.equal(
    calls.ticker.length,
    0,
    "no per-symbol fallback when bulk succeeds",
  );
});
...
test("depth concurrency never exceeds configured limit", async () => {
  const instruments = Array.from({ length: 20 }, (_, i) =>
    instrument(`SYM${i}USDT`),
  );
  let maxConcurrent = 0;
  let current = 0;
  ...
  const limit = 3;
  const { report } = await enrichWithOrderBooks(instruments, adapter, {
    depthLimit: 5,
    concurrency: limit,
  });
  assert.ok(
    maxConcurrent <= limit,
    `maxConcurrent ${maxConcurrent} > limit ${limit}`,
  );
...
test("unknown spread is rejected explicitly", () => {
  ...
  const validFactors = () => ({
    ...
    spread: {
      ...
      raw: null,
      normalized: 0,
      reason: "kein Spread bekannt",
      detail: {},
    },
    executionCost: {
      id: "executionCost" as const,
      available: false,
      raw: null,
      normalized: 0,
      reason: "no spread",
...
test("volume24h uses quote volume, not base volume", async () => {
  ...
  assert.equal(
    volumeBySymbol.get("BTCUSDT"),
    5_000_000,
    "must use quoteVol, not baseVol",
  );
...
test("integration: 2 with spread, 1 without → scanner funnel", async () => {
  ...
  for (const inst of [btc!, eth!, sol!]) {
    const res = checkEligibility(
      {
        instrument: inst,
        factors: makeFactors(inst) as any,
        candleCount: 100,
        regime: "NORMAL",
      },
      config,
  ...
  assert.ok(
    eligible.length >= 1,
    `expected ≥1 eligible, got ${eligible.length}`,
  );
...
// ── Gepoolter No-Bulk-/Lücken-Pfad: Determinismus trotz Parallelität (v1.37.0)

test("enrichWithTickers (ohne Bulk): parallele Einzelabrufe, Failure-Reihenfolge bleibt Eingangsreihenfolge", async () => {
  const symbolsList = ["AAAUSDT", "BBBUSDT", "CCCUSDT", "DDDUSDT"];
  const instruments = symbolsList.map((s) => instrument(s));
  const failing = new Set(["BBBUSDT", "DDDUSDT"]);
  // Spätere Fehler resolven BEWUSST zuerst — bei Seiteneffekten im Worker
  // würde die Failure-Reihenfolge dadurch kippen.
  const delay = (s: string) => (s === "DDDUSDT" ? 1 : s === "BBBUSDT" ? 20 : 5);
  const adapter = {
    getTicker: (symbol: string) =>
      new Promise<MarketTicker>((resolve, reject) => {
        setTimeout(() => {
          if (failing.has(symbol)) reject(new Error("ticker down"));
          else resolve(ticker(symbol, 2_000_000));
        }, delay(symbol));
      }),
  ...
  assert.equal(report.succeeded, 2);
  assert.deepEqual(
    report.failures.map((f) => f.symbol),
    ["BBBUSDT", "DDDUSDT"],
    "Failures werden eingangsgeordnet zusammengefaltet, nicht nach Latenz sortiert",
  );
  assert.equal(volumeBySymbol.get("AAAUSDT"), 2_000_000);
  assert.equal(volumeBySymbol.get("BBBUSDT"), null);
});

test("enrichWithTickers: Bulk-Lücken werden gepoolt gefüllt (kein serieller N+1-Pfad)", async () => {
  const symbolsList = ["AAAUSDT", "BBBUSDT", "CCCUSDT", "DDDUSDT"];
  const instruments = symbolsList.map((s) => instrument(s));
  let maxConcurrent = 0;
  let inFlight = 0;
  const adapter = {
    // Bulk liefert nur die ersten zwei.
    getTickers: async () => [
      ticker("AAAUSDT", 1_000_000),
      ticker("BBBUSDT", 1_000_000),
    ],
    getTicker: (symbol: string) =>
      new Promise<MarketTicker>((resolve) => {
        inFlight += 1;
        maxConcurrent = Math.max(maxConcurrent, inFlight);
        setTimeout(() => {
          inFlight -= 1;
          resolve(ticker(symbol, 3_000_000));
        }, 15);
      }),
  } as unknown as MarketDataAdapter;
  ...
  assert.equal(
    report.succeeded,
    4,
    "beide Lücken werden vom Einzel-Ticker geschlossen",
  );
  assert.equal(volumeBySymbol.get("CCCUSDT"), 3_000_000);
  assert.equal(volumeBySymbol.get("DDDUSDT"), 3_000_000);
  assert.ok(
    maxConcurrent >= 2,
