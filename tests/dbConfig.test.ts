...
test("scripts/setup-cachyos.sh: Passwort-Interpolation ist quote-/injection-sicher", () => {
  ...
  // Erster Fix: psql-Variable :'db_pass' (Quote-sicher, lief aber als
  // `-v db_pass=…` durch die Prozessliste). Aktueller Stand (SEC-PSQL,
  // v1.30.1): Passwort wird serverseitig Dollar-quotet
  // (pg_sql_quote_password) und das fertige SQL per STDIN an psql gereicht
  // — quote-sicher UND argv-frei.
  assert.ok(
    script.includes("pg_sql_quote_password"),
    "CREATE USER muss das Passwort über pg_sql_quote_password dollar-quoten",
  );
  assert.ok(
    /printf '%s\\n' "\$role_sql" \| pg_psql/.test(script),
    "Das CREATE-Statement muss per STDIN (Pipe) an psql gehen, nie als Argument",
  );
  // Kommentarzeilen dokumentieren die alte (verworfene) Form und bleiben
  // zulässig; geprüft wird nur ausführbarer Code.
  const codeLines = script
    .split("\n")
    .filter((line) => !line.trimStart().startsWith("#"));
  assert.ok(
    codeLines.every((line) => !line.includes("-v db_pass=")),
    "db_pass darf nie als psql -v-Argument übergeben werden (lesbar via ps aux)",
  );
  assert.ok(
...
test("src/db/index.ts: Import OHNE DATABASE_URL wirft nicht (next-build-fähig)", () => {
  ...
  const entry = pathToFileURL(resolve(process.cwd(), "src/db/index.ts")).href;
  const r = spawnSync(
    process.execPath,
    [
      "--import",
