...
| **Dashboard: „Sitzung abgelaufen — bitte neu anmelden."** | die `firm_session`-Cookie fehlt nach Dienst-Neustart/Secret-Rotation oder ist älter als 15 min; `GET /api/firm` antwortet `401`, weil es seit SEC-02 `firm.read` verlangt — **kein** Datenbankproblem (`/api/health` meldet `schemaReady:true`) | Token aus `.env` (`FIRM_API_TOKEN`) im Dashboard eintragen → **Anmelden**; der Firm-Status lädt seit v1.36.41 automatisch neu (kein `F5`). Bis v1.36.40 stand dort irreführend „Firm-Status nicht verfügbar (Datenbank)." — Details: **[HOWTO_LAN_SESSION.md](HOWTO_LAN_SESSION.md)** |
