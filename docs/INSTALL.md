...
| **Dashboard: „Sitzung abgelaufen — bitte neu anmelden.“** | `GET /api/firm` antwortet `401`, weil es seit SEC-02 `firm.read` verlangt — **kein** Datenbankproblem (`/api/health` meldet `schemaReady:true`). Seit v1.39.0 als Ursachen: Dienst-Neustart oder Secret-/Token-Rotation (entwertet alle Sitzungen), absolute Grenze `FIRM_SESSION_MAX_LIFE_S` erreicht, oder Idle-Frist abgelaufen **und** Nachfrist `FIRM_SESSION_GRACE_S` vorbei | Erst **Verlängern** im Hinweisbalken — innerhalb der Nachfrist heilt das ohne Token. Sonst Token aus `.env` (`FIRM_API_TOKEN`) eintragen → **Anmelden**; der Firm-Status lädt automatisch neu (seit v1.36.41, kein `F5`). Der Balken sagt außerdem, ob serverseitig überhaupt ein Token eingetragen ist (`Firm-API: eingetragen` / `KEIN Token gesetzt`). Bis v1.36.40 stand dort irreführend „Firm-Status nicht verfügbar (Datenbank).“ — Details: **[HOWTO_LAN_SESSION.md](HOWTO_LAN_SESSION.md)** |
| **Login gemeldet als erfolgreich, Sitzung bleibt „nicht angemeldet“** | fast immer plain-HTTP im LAN: beide Cookies tragen `Secure`, der Browser verwirft sie über `http://192.168.x.x:3369`. Seit v1.39.0 meldet das Dashboard genau das wörtlich (Diagnose nach dem Login) | App über TLS-Proxy (→6, Port 8443) oder über `localhost` öffnen. Gegencheck: `curl -i -X POST .../api/auth/login` zeigt die `Set-Cookie`-Zeilen, der Browser die Verwerfung; `GET /api/auth/status` zeigt `session.state` |
| **Balken: „KEIN Token gesetzt“** | in der Server-`.env` fehlt `FIRM_API_TOKEN`/`FIRM_ADMIN_TOKEN`/`FIRM_VIEWER_TOKEN` — das Dashboard kann das nicht ändern | Flag im Server-`.env` setzen, alle Instanzen neu starten, dann über den Balken anmelden (v1.39.0) |
