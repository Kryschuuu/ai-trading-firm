...
| GAP-03 | Trade-Journal mit Agenten-Attribution & Feedback-Loop | ★★★★★ | IN_PROGRESS | 1.43.0 | `arena/01a0b4c1-ai-trading-firm` | D1–D4 in Umsetzung: append-only `trade_journal` + `journal_agent_weights`, MAE/MFE (CANDLE_GAP/NO_DATA), Bayes-Glättung α=β=2 + minSample, `JOURNAL_FEEDBACK_MODE` off (Default)/monitor/enforce, `GET /api/firm/journal`. FIXED nach Merge | [PROMPT-03](../prompts/PROMPT-03-trade-journal-attribution.md) |
