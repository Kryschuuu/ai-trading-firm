...
| RMA-P1-06 | Trade-Attribution | FIXED | [P1-06](../prompts/PROMPT-P1-06-trade-attribution.md) | PR-PENDING (wird nach PR-Öffnung nachgetragen) | HEAD dieses Branches | v1.57.0 | `tests/tradeAttribution.test.ts` (20: Reconciliation LONG/SHORT/Gewinn/Verlust, Kosten bekannt/unbekannt, Alignment-/Enthaltungsregeln, Konflikte, Determinismus/Golden, Negative Paths), `tests/tradeAttribution.db.test.ts` (7: Roundtrip, Idempotenz Retry/Restart, Methodenwechsel erhält alte Zeilen, Backfill v1 ⇒ UNATTRIBUTABLE + zweiter Lauf leer, Aggregate-Reconciliation + Coverage, Close-Wiring + Disable-Flag, Migration idempotent + Append-only-Trigger auf eigener Wegwerf-Postgres), `tests/tradeAttribution.api.test.ts` (4: 400-Verträge, Dimensionen, no-store); `tests/tradeJournal.test.ts` auf Snapshot v2 erweitert; typecheck/lint/docs:validate grün |
