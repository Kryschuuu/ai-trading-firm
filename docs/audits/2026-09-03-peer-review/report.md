...
> Arena-Prompt-Datei unter [`audit-remediation/`](./findings/) (`README.md` = Index).
...
Tabelle; überspringt sich selbst, wenn keine PostgreSQL erreichbar ist). **H7 ist in v1.36.20 gefixt** (Kill-Switch/Flatten: `flattenAll` löst den Notfall-Broker jetzt venue-bewusst über `resolveEmergencyBroker()` auf — neue `EmergencyBroker`-Schnittstelle `cancelAllOpenOrders → closeAllPositions → verifyFlat`, erfüllt vom `PaperBroker` und den Live-`BrokerExecutionEngine`s (Bitunix `cancel_all_orders`/`close_all_position`, Alpaca `DELETE /v2/orders`/`DELETE /v2/positions`); `/api/firm/kill` läuft die Sequenz VOR `killSwitch.pull()` und belegt die Glattheit im `FLATTEN_ALL`-Audit (`mode`/`venue`/`flat`; Paper-Default: „paper-only flatten (live disabled)“). Test: `tests/h7.emergencyFlatten.test.ts` — Mock-`EmergencyBroker`, Reihenfolge cancel→close→verify vor Arm, Retry + NOT_FLAT ohne Wurf). **H5 ist in v1.36.6 gefixt** (Proposal-only-Phasen; nur der Executor führt eine genehmigte Proposal aus). **H6 ist in v1.36.7 gefixt** (echte Approval-Chain; Executor führt nur noch server-validierte APPROVED Proposals aus; menschliche Freigabe via Endpoint). **H9 ist in v1.36.8 gefixt** (Guardrail-Numerik fail-closed: `requireFinitePositive` wirft `RiskValidationError` bei NaN/Infinity/≤0; negatives Equity wird blockiert statt geklemmt; alle Caller lehnen mit `INVALID_EQUITY`/`INVALID_LEVERAGE`/`INVALID_NOTIONAL` ab). **H8 ist in v1.36.10 gefixt** (kanonische `BrokerAccount`-Zerlegung walletBalance/availableCash/usedMargin/maintenanceMargin/unrealizedPnl; Bitunix-Equity = `walletBalance + realizedPnl + unrealizedPnl` statt `available + uPnL`). **B1 ist in v1.36.11 gefixt** (Bitunix-SL/TP-Geometrie: `serializePlaceOrder` lehnt semantisch falsche Stop/Take-Staffelung relativ zum Entry ab). **B2 ist in v1.36.12 gefixt** (Bitunix-Positionsseite: `getPositions` validiert `side` auf `LONG`/`SHORT` und verwirft den Rest — Zähler + Warnung statt stiller `LONG`-Default; `qty<=0`-Zeilen scheiden vorher aus). **C1 ist in v.1.36.13 gefixt** (Auth-Modus `AUTH_MODE = local-open | token-required` in `src/auth/authMode.ts`; Produktion ohne Token verweigert den Start über den Boot-Guard in `src/instrumentation.ts` mit `ConfigurationError: AUTH_NOT_CONFIGURED`; `checkApiToken`/`resolveAuth` fragen denselben Modus ab, und wo nur ein Admin-/Viewer-Token existiert, entscheidet die Permission `firm.write` statt „offen“). **C2 ist in v1.36.14 gefixt** (Rate-Limit-Identität: `src/lib/clientIp.ts` als einzige Quelle für Firm- und Credential-Limiter — `TRUSTED_PROXY_IPS` + proxy-gesetztes `x-verified-ip`, `x-forwarded-for` nur hinter verifiziertem Trusted-Proxy-Peer und rightmost-untrusted, `x-real-ip` nie; ohne Vertrauen Socket-Adresse bzw. Konstante `local`. Dazu globaler IP-unabhängiger Credential-Deckel `BROKER_CREDENTIAL_GLOBAL_RATE_LIMIT` (20/min) und exponentieller Backoff ab dem 3. Fehlversuch (2 s → 15 min) — Kill-Switch bewusst ausgenommen; Diagnose über `GET /api/auth/me → rateLimitIdentity`). **C3 ist in v1.36.15 gefixt** (Kill-Switch-Disarm stärker als Arm: `src/app/api/firm/kill/route.ts` splittet den Guard — Arm bleibt `guardWrite`/Operator; Disarm verlangt `requirePermission(req,"live.gate")` [ADMIN] + CSRF (`checkCsrfGuard`) + einen kurzlebigen single-use Nonce (≤ 60 s) aus dem neuen `GET /api/firm/kill/challenge` (`src/app/api/firm/kill/challenge/route.ts`); Nonce-Speicher `src/lib/disarmChallenge.ts`, atomar per `consumeDisarmNonce`; fehlend/abgelaufen/wiederverwendet ⇒ 403; erst danach `killSwitch.disarm()` + **CRITICAL**-Audit `KILL_SWITCH_DISARMED` mit Actor + Nonce, Audit-View erwartet jetzt `CRITICAL`). **C4 ist in v1.36.16 gefixt** (Control-Plane-Zustand persistiert: neue Tabelle `venue_control_state` [venue PK, configured, connected, permissions jsonb, live_enabled, last_probe, connection_state, discovery_state, discovery_count, discovery_last_sync, last_error, layers jsonb, updated_at]; `src/brokers/control-plane/stateStore.ts` mit `DbControlStateRepository`/`MemoryControlStateRepository`; `writeState()` upsertet, `loadState()` liest Cache → DB → Initialzustand (lazy persistiert); `getStatus`/`readVenueControlStatePublic` spiegeln nach Neustart den letzten bekannten Zustand, Boot-Warm-up in `instrumentation.ts`; Live wird beim Laden immer neu aus dem Enforcer projiziert; ohne Tabelle Memory-Fallback + eine Log-Warnung — additiv, `npx drizzle-kit push`). **S1 ist in v1.36.18 gefixt** (klassifizierte Audit-Senke `src/lib/auditSink.ts`: Sicherheits-Audits mit Retry + exponentiellem Backoff, persistentem Spool `data/audit-spool` als at-least-once-Fallback und CRITICAL-Alarm statt leerem `catch`; `failClosed` für Credential-Store, Kill-Switch-Disarm und Proposal-Freigabe — ohne durablen Beleg bleibt die Mutation aus; Telemetrie-Pfade bleiben best-effort, loggen und zählen aber; `/api/health → audit`, Operations-Center-Kennzahlen und `audit_*`-Counter in der Prometheus-Exposition). **H10 ist in v1.36.21 gefixt** (adaptives Risk fail-closed: neues Regime `UNKNOWN` — fehlende/fehlerhafte/zu alte Bewertung ⇒ `regime:"UNKNOWN"` statt stillem NORMAL-Fallback, wirksames Limit klemmt auf den Code-Boden `LIMIT_CEILINGS.maxRiskPerTrade[0]` (0.2 %), Wechsel als WARN-Event auditiert; `runAgentTurn` blockt Neupositionen bei UNKNOWN wie bei EXTREME mit Guardrail `ADAPTIVE_RISK_UNKNOWN`/`ADAPTIVE_RISK_EXTREME`, Trace zeigt `ADAPTIVES-RISIKO ok=false` + `ADAPTIVES-RISIKO-GATE`; deaktivierte Systeme bleiben bewusst NORMAL. Test: `tests/h10.adaptiveUnknown.test.ts` — `resolveAdaptiveUnknown`/`adaptiveUnknownFactor` unit, UNKNOWN-Pfad + Erholung in `updateAdaptiveRisk`, `adaptiveAllowsNewPositions`-Matrix und DB-gegateter `runAgentTurn`-Akzeptanztest). **S2 ist in v1.36.22 gefixt** (zentrale `stateRegistry`: ALLE 14 Cross-Cutting-Singletons [Engine, Control Plane, Risk-Guard, Broker-Factory, API-Auth] hängen jetzt an EINEM dependency-freien `globalThis`-Namensraum mit typisierten Accessoren und dokumentierter Wahrheitsquelle DB vs. RAM; die verstreuten Reset-Pfade entfallen, das Test-Harness resettet über EINE Funktion `__resetAllSingletonsForTests()` einschließlich der früher separat zurückgesetzten Control-Plane- und Rate-Limiter-Singletons; `resetControlPlaneForTests` bleibt als Repo-/DI-Installations-Hook bestehen, `resetRuntimeLimits` resettet bewusst nur Basis-Limits — Adaptivfaktor überlebt, dokumentiert und getestet. Test: `tests/stateRegistry.test.ts` — registrierte Slots, lazy/idempotente Accessoren, ein Reset über alle Besitzer, Lifecycle-Doku nennt `positions`/`kill_switches`/`risk_config`/`venue_control_state`/`proposals` als DB-Wahrheit). **W2 ist in v1.36.24 gefixt** (Prompt-Editor ohne Versionskontrolle: `PUT /api/firm/agents` aktualisierte `systemPrompt` ohne jede Version-Prüfung — zwei Browser-Edits überschrieben sich still (last-write-wins). Neu: Spalte `agents.version` (DEFAULT 1, additiv; idempotente Migration `drizzle/2026-09-05_w2_agents_version.sql`); `validatePromptInput` verlangt `expectedVersion` (positive ganze Zahl); der UPDATE greift nur mit `WHERE id = agentId AND version = expectedVersion` und inkrementiert atomar (`version = version + 1`); 0 betroffene Zeilen ⇒ 409 CONFLICT inkl. `currentVersion`; Erfolg liefert die neue Version zurück; der `AGENT_PROMPT_UPDATED`-Audit bleibt. Die UI (PromptPanel) sendet die beim Laden gesehene Version, zeigt sie im Kopf, behandelt 409 mit „Konflikt: neu laden“ — Entwurf verwerfen, Fremdstand neu laden. Test: `tests/w2.promptVersioning.test.ts` — Validator-Unit, Statik-Greps und DB-gegatete Akzeptanz mit dem Zwei-PUTs-Szenario). **W1 ist in v1.36.23 gefixt** (Workshop: der FIRM_API_TOKEN lag dauerhaft in `localStorage` (`saveToken` → `window.localStorage.setItem("firmToken", …)`), jedes XSS im Origin las ihn und bekam API-Zugriff. Neu: `POST /api/auth/login` verifiziert den Token serverseitig und setzt `firm_session` (HttpOnly, Secure, SameSite=Strict, Path=/, Max-Age=900) plus `firm_csrf` für Double-Submit-CSRF — stateless HMAC-Session in `src/lib/authSession.ts` (Schlüssel: `FIRM_SESSION_SECRET` oder aus den konfigurierten Tokens abgeleitet; Produktion über plain-HTTP ⇒ fail-closed `SESSION_HTTPS_REQUIRED`). `resolveAuth` akzeptiert die Session-Cookie (neue `ActorSource "api-session"`), `checkApiToken` lässt Operator-/Admin-Sessions schreiben, `checkCsrfGuard` prüft double-submit. Client liest nirgends mehr `firmToken` aus `localStorage` (`apiClient.ts`, `controlPlane.ts`, `liveGate.ts`, `FirmDashboard` → Login-Aufruf, Altbestand-Entfernung über `browserSession.ts`). Test: `tests/w1.sessionCookie.test.ts` — Cookie-Flags, Signatur/Manipulation/Ablauf, resolveAuth/checkApiToken/checkCsrfGuard, Login-Route inkl. HTTPS-Enforcement, Statik-Greps).

| ID | Bereich | Severity | Status (validiert) | Prompt |
|----|---------|----------|--------------------|--------|
| H1 | Handelslogik | CRITICAL | ✅ bereits gefixt (v1.36.2) | [H1](./findings/H1-risk-notional.md) |
| H2 | Handelslogik | CRITICAL | ✅ **gefixt v.1.36.19** | [H2](./findings/H2-atomicity.md) |
| H3 | Handelslogik | CRITICAL | ✅ gefixt (v1.36.4) | [H3](./findings/H3-order-status.md) |
| H4 | Handelslogik/Broker | CRITICAL | ✅ gefixt (v1.36.5) | [H4](./findings/H4-idempotency.md) |
| H5 | Handelslogik | CRITICAL | ✅ gefixt (v1.36.6) | [H5](./findings/H5-pipeline-trading.md) |
| H6 | Handelslogik | CRITICAL | ✅ gefixt (v1.36.7) | [H6](./findings/H6-approval-chain.md) |
| H7 | Handelslogik/Control | HIGH | ✅ **gefixt v.1.36.20** | [H7](./findings/H7-live-kill.md) |
| H8 | Brokers/Venues | HIGH | ✅ gefixt (v1.36.10) | [H8](./findings/H8-bitunix-equity.md) |
| H9 | Handelslogik | HIGH | ✅ gefixt (v1.36.8) | [H9](./findings/H9-finite-guardrails.md) |
| H10 | Handelslogik | HIGH | ✅ **gefixt v.1.36.21** | [H10](./findings/H10-adaptive-failopen.md) |
| W1 | Workshop | HIGH | ✅ **gefixt v.1.36.23** | [W1](./findings/W1-localstorage.md) |
| W2 | Workshop | MEDIUM | ✅ **gefixt v.1.36.24** | [W2](./findings/W2-prompt-versioning.md) |
| C1 | Control Panel | HIGH | ✅ **gefixt v.1.36.13** | [C1](./findings/C1-open-mode.md) |
| C2 | Control Panel | MED/HIGH | ✅ **gefixt v.1.36.14** | [C2](./findings/C2-forwarded-ip.md) |
| C3 | Control Panel | HIGH | ✅ **gefixt v.1.36.15** | [C3](./findings/C3-kill-disarm.md) |
| C4 | Control Panel | MEDIUM | ✅ **gefixt v.1.36.16** | [C4](./findings/C4-control-state-persistence.md) |
| B1 | Brokers/Venues | HIGH | ✅ gefixt (v1.36.11) | [B1](./findings/B1-sl-tp-geometry.md) |
| B2 | Brokers/Venues | MEDIUM | ✅ gefixt (v1.36.12) | [B2](./findings/B2-side-fallback.md) |
| S1 | Sonstiges | MEDIUM | ✅ **gefixt v.1.36.18** | [S1](./findings/S1-audit-reliability.md) |
| S2 | Sonstiges | MEDIUM | ✅ **gefixt v.1.36.22** | [S2](./findings/S2-singleton-consistency.md) |
...
4. **Architektur/Live-Bereitschaft:** H2 ✅ **gefixt v.1.36.19** — H7 ✅ **gefixt v.1.36.20** — H10 ✅ **gefixt v.1.36.21** — S2 ✅ **gefixt v.1.36.22**
5. **Workshop:** W1 ✅ **gefixt v.1.36.23** — W2 ✅ **gefixt v.1.36.24**

Jeder Schritt ist unabhängig; H1 entfällt (bereits gefixt). Stand 2026-09-04 sind aus der
...
**gefixt v.1.36.15** — und **C4 (v1.36.16)** — **gefixt v.1.36.16** — abgeschlossen (S1 ✅
**gefixt v.1.36.18**); aus der Architektur/Live-Bereitschaft sind **H2 (v1.36.19)**, **H7 (v1.36.20)**,
**H10 (v1.36.21)**, **S2 (v1.36.22)**, **W1 (v1.36.23)** und **W2 (v1.36.24)** abgeschlossen — alle validierten Befunde sind behoben.
...
**H2=v1.36.19**, **H7=v1.36.20**, **H10=v1.36.21**, **S2=v1.36.22**, **W1=v1.36.23**, **W2=v1.36.24**.
Siehe `CHANGELOG.md` und `docs/CHANGELOG.md` (`[1.36.24]`). Die Versions-Hinweise der einzelnen Befunde nennen teils noch
ältere Nummern (C1–C4 ursprünglich `1.36.3`) — maßgeblich ist die Serie oben.

## Verwandte Dokumente

- [Security-Audit](../../security/SECURITY_AUDIT.md)
- [`README.md`](./findings/README.md) — Prompt-Index + Validierung
- `CHANGELOG.md` / `docs/CHANGELOG.md` — `[1.36.24]`-Eintrag
