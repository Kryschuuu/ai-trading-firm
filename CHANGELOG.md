...
**MEDIUM, Doku & Struktur.** Grundlegendes Aufräumen des Repositories nach 20+ Audit-Findings (SEC-01 PrivEsc, SEC-02 unprotected APIs, SEC-03/04 deps, H1-H10, C1-C4, B1/B2, S1/S2, W1/W2):

- **Neue skalierbare Struktur:**
  - `docs/audits/` — alle Audits chronologisch `YYYY-MM-DD-<quelle>-<kurzname>` (TEMPLATE + README + TRACKING-Modell), CLOSED `2026-09-03-peer-review` (H1-H10 etc.), OPEN `2026-09-05-security-review-gpt01` (SEC-01..SEC-07)
  - `docs/peer-reviews/` — 3 Reviews (`2026-08-26-live-trading-readiness`, `bitunix-execution`, `routing-overrides`) mit `review.md` + `patches/PATCH-001..` + README
  - `docs/security/` — kanonisch `SECURITY_AUDIT.md` + README (aggregiert offene Critical/High)
  - `docs/archive/task-plans/` — 8 historische `task-*.md`
  - `docs/ci/` — Quelle der Workflows (Arena-Bot darf `.github/workflows` nicht schreiben) + README mit Install-Anleitung

- **Duplikate konsolidiert (keine multiplen Wahrheiten mehr):**
  - `audit-remediation/` Root (21 Files) → `docs/audits/2026-09-03-peer-review/findings/`
  - `docs/PEER_REVIEW_*.md` (3) → `docs/peer-reviews/*/review.md` + Stubs (Weiterleitung, <1500 Zeichen)
  - `docs/AUDIT_REMEDIATION_2026-09.md` → `docs/audits/2026-09-03-peer-review/report.md` + Stub
  - `docs/SECURITY_AUDIT.md` → `docs/security/SECURITY_AUDIT.md` + Stub
  - `CHANGELOG.md` Root (5833 Zeilen detailliert) kanonisch, `docs/CHANGELOG.md` Stub
  - `CONFIGURATION.md` Root Flag-Ref kanonisch, `docs/INSTALL.md` CachyOS-Guide, `INSTALL.md` Root Wrapper

- **Naming & Verknüpfung:**
  - Audit-Ordner `YYYY-MM-DD-<quelle>-<kurzname>`, Finding `<ID>-<slug>.md` (ID aus Original), Patch `PATCH-<NNN>-<slug>.md`
  - Chronologisch primär + `remediation/TRACKING.md` Status (OPEN/FIXED), `security/README.md` aggregiert
  - Bidirektionale Links Finding↔Patch mit Frontmatter `related_findings`, `audit`, `status: IMPLEMENTED`
  - Dokumentiert in `docs/REPOSITORY_STRUCTURE.md` (Zielbild, Naming, Migration, Duplikat-Tabelle, Wartbarkeit)

- **Code & CI:**
  - `src/lib/docsCatalog.ts` — Encoding-Bug fix: `PEER_REVIEW_*.md → docs/peer-reviews/*/review.md` enthielt `*/` das Block-Kommentar schloss → TS1109; ersetzt, typecheck grün
  - `scripts/docs-validate.ts` — `checkLinks()` ignoriert Code-Fences + Inline-Code + Platzhalter `...`, Stub-Handling, Version-Check nur Root CHANGELOG
  - `docs/audits/2026-09-03-peer-review/findings/README.md` neu als sauberer Index (20 Findings)
  - `docs/ARCHITECTURE.md` Link fix `docs/CHANGELOG.md` → `CHANGELOG.md im Root`

- **Validiert:** `npm ci` (503 packages) + `npm run docs:validate` → 8 Checks OK grün + `npm run typecheck` → 0 Fehler

### Version

`1.36.26` (package.json, Root CHANGELOG, docs/README, CONFIGURATION, INSTALL, README).
